...
function adaptFailedDeals(report) {
  return {
    recovery_candidates: ensureArray(report?.failed_deal_reanimation?.cards).map((card) => ({
      ...
      reason_type: card?.reason_type || card?.failure_reason_type || card?.failure_category || card?.reason_bucket || "",
      failure_category: card?.failure_category || card?.failure_reason_type || card?.reason_type || card?.reason_bucket || "",
      failure_reason: card?.failure_reason || card?.failure_comment || "",
